...
<%@ include file="/WEB-INF/jsp/cmmn/noticeModal.jsp" %>
...
<script type="text/javascript">
	...
	function calendarLoad(year, month) {
		 ...
		 $.ajax({
			url: "<c:url value='/admin/menuPlanLoad'/>",
			type: "POST",
			data : {
				year : year,
				month : month
			...
			success : function(data){
				...
				str+="<div style='float: left; width:33%;'><a class='nav-link' href='#'><i class='fas fa-caret-left' style='color:black; font-size:40px;' onclick='calendarLoad(" + data.today_info.before_year + "," + data.today_info.before_month + ")'></i></a></div>"; 
				str+="<div style='float: left; width:33%;'><span class='this_month'>&nbsp;";
				...
				if(data.today_info.search_month < 10){
					...
					str+="0" + data.today_info.search_month + "월 식단표</span></div>";
				} else{
					str+=data.today_info.search_month + "월 식단표</span></div>";
				}
				<!-- 다음달 -->
				//str+="<a class='before_after_month' onclick='calendarLoad(" + data.today_info.after_year + "," + data.today_info.after_month + ")'>";
				str+="<div style='float: left; width:33%;'><a class='nav-link' href='#'><i class='fas fa-caret-right' style='color:black; font-size:40px;' onclick='calendarLoad(" + data.today_info.after_year + "," + data.today_info.after_month + ")'></i></a></div>"; 
				<!-- 다음해 -->
